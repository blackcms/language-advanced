...
class HookServiceProvider extends ServiceProvider
{
    public function boot()
    {
        ...
        add_filter(
            ...
            [$this, "getDataLanguageBeforeShow"],
            1135,
            4
        );
        add_filter(
        ...
        add_filter(
            ...
            [$this, "checkItemLanguageBeforeGetListItem"],
            1134,
            2
        );
        ...
        add_filter(
            ...
            function ($form, $data) {
                if (is_in_admin() &&
                    ...
                    $columns = LanguageAdvancedManager::getTranslatableColumns(
                        $data
                    );
                    ...
                    if (Language::getCurrentAdminLocaleCode() !=
                        Language::getDefaultLocaleCode()
                    ) {
                        $refLang =
                            "?ref_lang=" .
                    ...
                    $form
                        ->setFormOption(
                            "url",
                            route("language-advanced.save", $data->id) .
                                $refLang
                        )
                        ...
                        foreach ($metaBox as $context => $position) {
                            foreach ($position as $item) {
                                foreach (array_keys($item) as $key) {
                                    if (LanguageAdvancedManager::isTranslatableMetaBox(
                                        $key
                                    )
                                    ) {
                                        continue;
        ...
        add_filter(
            ...
            function ($html = null, $object = null) {
                if (is_in_admin() &&
                    request()->input("ref_lang") &&
                    Language::getCurrentAdminLocaleCode() !=
                        Language::getDefaultLocaleCode() &&
                    ...
                    Assets::addStylesDirectly(
                        "vendor/core/packages/slug/css/slug.css"
                    );

                    $prefix = SlugHelper::getPrefix(get_class($object));

                    return view(
                        "addons/language-advanced::slug",
        ...
        add_filter(
            "stored_meta_box_key",
            function (string $key, $object) {
                $locale = is_in_admin()
                    ? Language::getCurrentAdminLocaleCode()
                    : Language::getCurrentLocaleCode();

                $translatableColumns = LanguageAdvancedManager::getTranslatableColumns(
                    $object
                );
    ...
    public function languageMetaField()
    {
        ...
        return view(
            "addons/language-advanced::language-box",
            compact("args", "languages", "currentLanguage", "route")
        )->render();
    }
    ...
    public function checkCurrentLanguage($languages)
    {
        ...
        foreach ($languages as $language) {
            if (($request->input("ref_lang") &&
                    $language->lang_code == $request->input("ref_lang")) ||
    ...
    public function getDataLanguageBeforeShow($data, $model)
    {
        ...
        if (is_in_admin() &&
            $currentAdminLocale == Language::getDefaultLocaleCode()
        ) {
            return $data;
        }
        ...
        if ($language) {
            ...
            $data = $data
                ...
                ->join(
                    $translationTable,
                    $translationTable . "." . $table . "_id",
                    $table . ".id"
                );

            $data = $data->where(
                $translationTable . ".lang_code",
                $currentAdminLocale
            );
        }
    ...
    public function addLanguageColumn($data, $model)
    {
        if ($model && LanguageAdvancedManager::isSupported($model)) {
            ...
            return $data->addColumn("language", function ($item) use (
                ...
                return view(
                    "addons/language-advanced::language-column",
                    compact("item", "route", "languages")
                )->render();
            });
        }
