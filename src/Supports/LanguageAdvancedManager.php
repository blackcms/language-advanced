...
namespace BlackCMS\LanguageAdvanced\Supports;

use BlackCMS\Base\Models\BaseModel;
use BlackCMS\Gallery\Models\GalleryMeta;
use BlackCMS\LanguageAdvanced\Models\PageTranslation;
...
class LanguageAdvancedManager
{
    ...
    {
        return config("addons.language-advanced.general.supported", []);
    }
    ...
    {
        return config("addons.language-advanced.general", []);
    }
    ...
    {
        config([
            "addons.language-advanced.general.supported" => array_merge(
                self::getSupported(),
                [$model => $columns]
            ),
        ]);
    ...
    public static function delete($object): bool
    {
        ...
        DB::table($table)
            ->where([$object->getTable() . "_id" => $object->id])
            ->delete();
    ...
    public static function isTranslatableMetaBox(string $metaBoxKey): bool
    {
        return in_array(
            $metaBoxKey,
            Arr::get(self::getConfigs(), "translatable_meta_boxes", [])
        );
    }
    ...
    public static function addTranslatableMetaBox(string $metaBoxKey): bool
    {
        ...
        config([
            "addons.language-advanced.general.translatable_meta_boxes" => $metaBoxes,
        ]);
